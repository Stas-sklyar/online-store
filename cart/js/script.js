...
function plusOneGood() {
    ...
    let priceGood = parseInt(arrowIcon.parentNode.parentNode.querySelector(".info-item-order__price").textContent);
    let totalPriceForThisGood = arrowIcon.parentNode.parentNode.querySelector(".item-order-total-price__price");

    previousInput.value++;
    totalPriceForThisGood.textContent = priceGood * previousInput.value;
    calcTotalPriceForAllGoods();
}
...
function minusOneGood() {
    ...
    if (nextInput.value <= 0) {
        ...
        nextInput.value--;
        totalPriceForThisGood.textContent = priceGood * nextInput.value;
        calcTotalPriceForAllGoods();
    }
}

function removeItemOrder() {
    let tempItemOrder = event.currentTarget.parentNode;
    let totalPriceForThisGood = tempItemOrder.querySelector(".item-order-total-price__price");
    let totalPriceAllGoods = document.querySelector(".total-order__total-price");

    totalPriceAllGoods.textContent = (parseInt(totalPriceAllGoods.textContent) - parseInt(totalPriceForThisGood.textContent));
    tempItemOrder.style.animationName = "fadeOut";
    displayNone(tempItemOrder);
    totalPriceForThisGood.textContent = "0";

}

function calcTotalPriceForAllGoods() {
    let totalPriceAllGoods = document.querySelector(".total-order__total-price");
    let totalSumAllGoods = 0;
    let allValueForTotalSum = document.querySelectorAll(".item-order-total-price__price");

    for (let valueItemGoods of allValueForTotalSum) {
        totalSumAllGoods += parseInt(valueItemGoods.textContent);
    }

    totalPriceAllGoods.textContent = totalSumAllGoods;
}
